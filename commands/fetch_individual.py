...
def fetch_individual_data(individual_id: str, force: bool = False):
    ...
    existing_data = (
        db.client.collection("rawIndividualContributions").document(individual_id).get()
    )
    if existing_data.exists and not force:
        existing_contribs = existing_data.to_dict().get("contributions", [])
        if existing_contribs:
            print(
                f"ℹ️  Individual '{individual_id}' already has {len(existing_contribs)} contributions. Use --force to refetch."
            )
            return {
    ...
    try:
        ...
        if "company" in individual_data and individual_data["company"]:
            logging.info(
                f"Selectively updating companies: {individual_data['company']}"
            )

            # Use selective update for daily operations efficiency
            company_new_recipients = update_specific_companies(
                db, individual_data, individual_id
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Fetch contribution data for an individual"
    )
    parser.add_argument("--id", required=True, help="Individual ID to fetch data for")
    parser.add_argument(
        "--force", action="store_true", help="Refetch even if data exists"
    )
    ...
    try:
        ...
        if result.get("skipped"):
            print(
                f"⏭️  Skipped '{args.id}' (already has {result['existing_contributions']} contributions)"
            )
        else:
            ...
            if result.get("companies_updated"):
                ...
                print(
                    f"🏢 Updated {len(companies)} company(ies), found {len(result.get('company_new_recipients', []))} new company recipients"
                )
